...
<script lang="ts">
	/**
	 * Bottom of the reel frame: THREE SEPARATE wells
	 * (WAYS | FREE SPINS / info marquee | WIN) — never one continuous bar.
	 * Asset: frame_bottom_compartments.png — THE WHITE ROOM padded/clinical rail
	 * (NOT Madam Mirror gold gothic filigree).
	 *
	 * Outer size lockstep with BoardFrame.svelte (GOLD_INNER / FRAME_SCALE / GAP).
	 * Do not reintroduce bak-locked FRAME_INSETS that thicken into a double frame.
	 */
	...
	// Inner BLACK screen of each well (opaque lum<35), NOT the riveted steel
	// bezel. Measured on frame_bottom_compartments.png @1524×462, then inset
	// ~10px so text/mask stay clear of the metal lip.
	const WELL = {
		y0: 0.178,
		y1: 0.752,
		left: { x0: 0.112, x1: 0.294 },
		center: { x0: 0.412, x1: 0.596 },
		right: { x0: 0.715, x1: 0.888 },
	};
	...
	const layout = $derived.by(() => {
		...
		// WELL fractions are already the padded black screen — no extra shrink.
		const wellW = (frac: { x0: number; x1: number }) => (frac.x1 - frac.x0) * railW;
		const wellH = (WELL.y1 - WELL.y0) * railH * 0.9;
...
<MainContainer>
	{@const L = layout}
	<!-- ONE regenerated bottom morph: 3 separated compartments under the frame lip -->
	<Sprite
	...
	{#each L.slots as s (s.text + String(s.show))}
		{#if s.show}
			<!-- Clip WAYS / FREE SPINS / WIN to the black well (not the steel bezel). -->
