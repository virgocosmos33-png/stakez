...
<script lang="ts">
	...
	const runSplit = async () => {
		...
		const swipeMs = CLAW_MS * CLAW_IMPACT;
		...
		await playClaw(() => {
			...
			const apart = splitProgress.set(1, { duration: 150, easing: backOut });
...
{#snippet clawStrike()}
	{#if clawT >= 0}
		...
		{#if smear > 0.02}
			{@render clawHand(t, alpha * 0.3 * smear, -CARD_H * 0.1 * smear, 0xffffff)}
			{@render clawHand(t, alpha * 0.16 * smear, -CARD_H * 0.19 * smear, 0xffffff)}
		{/if}
		{@render clawHand(t, alpha, 0, 0xffffff)}
	{/if}
{/snippet}
