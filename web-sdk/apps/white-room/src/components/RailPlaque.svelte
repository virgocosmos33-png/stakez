<script lang="ts">
	...
	// Used by WaysCounter + rail WIN. Caption + value FittedText inside glass.
	// Font/tint from config.fx.winAmountFont — never Mining-Mayhem silver slab.
	type Props = {
		caption: string;
		value: string;
		frameW: number;
	...
	<FadeContainer {show} {x} {y}>
		<Container scale={pop}>
			...
			<FittedText
				...
				tint={CAPTION_TINT}
				text={caption}
				style={{ fontFamily: AMOUNT_FAMILY, fontSize: textH * 0.62, letterSpacing: 2 }}
			/>
			...
			<FittedText
				...
				tint={AMOUNT_TINT}
				text={value}
				style={{ fontFamily: AMOUNT_FAMILY, fontSize: textH, letterSpacing: 1 }}
			/>
		</Container>
	</FadeContainer>
</MainContainer>
