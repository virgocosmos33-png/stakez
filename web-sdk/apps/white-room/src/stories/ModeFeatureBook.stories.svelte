...
<!-- CELL FRACTURE (10x): one guaranteed observation pane -->
<Story
	...
	args={templateArgs({
		...
		action: async () => {
			...
			console.log('Running CELL FRACTURE (feature1) book: 1 guaranteed mirror');
			await playBet({ ...data, state: data.events });
		},
	})}
	{template}
...
<!-- TWIN MIRRORS (20x): two guaranteed observation panes -->
<Story
...
<!-- TRIPLE MIRRORS (40x): three guaranteed observation panes -->
<Story
...
<!-- Wild Reel showcases now live in SpecialSymbols.stories.svelte -->
