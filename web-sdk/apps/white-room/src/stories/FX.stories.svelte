...
<!-- fx.splitTelegraph → MirrorShatter.svelte: intakeShot buckles + tube stubs
	+ clipboard stamps toward fracture targets (ZERO glass knives) -->
<Story
	name="splitTelegraph"
	args={templateArgs({
		skipLoadingScreen: true,
...
<!-- fx.splitShatter → MirrorShatter.svelte: intakeStamp lock, then paddedTear
	foam/strap debris + fluorescent blackout (ZERO falling glass) -->
<Story
	name="splitShatter"
	args={templateArgs({
		skipLoadingScreen: true,
...
<!-- fx.mirrorBreak → MirrorShatter.svelte: intakeStamp clipboard lock + tube frame -->
<Story
	name="mirrorBreak"
	args={templateArgs({
		skipLoadingScreen: true,
...
<!-- fx.splitAnimation → ApparitionOverlay.svelte runSplit/pulse: plays the REAL
	mirrorBurst book event (stamp lock → padded tear → intake shot → fluorescent
	scan cut, stamp flare, perforation tear, panes parting) on its recorded board -->
<Story
	name="splitAnimation"
	args={templateArgs({
		skipLoadingScreen: true,
...
<!-- fx.framePlasma → BoardFramePlasma KILLED (unmounted). Was not the dashed
	grey-lip source — that was baked PNG quilt stitches. Story kept as no-op.
	legacy note: purple flame ring licking off
