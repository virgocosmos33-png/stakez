// Ambient FLIES crawling the white room — trimmed from the classic screen-bugs
// snippet: spiders removed, capped at 5 small flies, sprite served locally
// (Stake Engine XSS policy: no external requests), z-index kept low so they
// stay background dressing under the HUD.
"use strict";
...
var BugDispatch = {
    ...
    options: {
        ...
        maxSmallTurnDeg: 10,
        maxWiggleDeg: 5,
        imageSprite: '/assets/sprites/bugs/fly_sprite.png',
        bugWidth: 13,
        bugHeight: 14,
        num_frames: 5,
...
var Bug = {
    ...
    makeBug: function() {
        if (!this.bug && this.active) {
            ...
            bug.style.top = 0;
            bug.style.left = 0;
            // background dressing: above the game canvas, under the HUD/UI
            bug.style.zIndex = '2';
...
// a handful of ambient flies, nothing else (spiders removed)
new BugController({
    minBugs: 2,
    maxBugs: 5,
    mouseOver: 'fly'
