...
// and the 30000x wincap is WHITEOUT / MAX WIN.
//
// Titles follow The Patient's intake arc (top symbol — black hair, force vest):
// INTAKE → RESTRAINT → STRUGGLE → BREAKOUT → SCRATCH → WHITEOUT.
// Scatters keep their own words (MEMORY…OBLIVION); celebrations do not reuse them.
...
export const winCelebrationTiers: WinCelebrationData[] = [
	...
	// Big tiers: WinCelebration.svelte drives bgm_celeb_1..6 stage-by-stage.
	// Seed a matching bed here so setWin / freeSpinOutro never fall back to
	// Madam Mirror bgm_winlevel_* before the takeover mounts.
	{
		tier: 2,
		alias: 'big',
		type: 'big',
