...
// drawRestraintStraps REMOVED with PlasmaLiner: the dashed strap outline traced
// around winning cell groups read as broken white shapes over the art.
...
/**
 * Fast specular MIRROR glint over a winning card: one soft diagonal light
 * band (halo -> body -> hot core, plus a thin trailing line) sliding across
 * the face, like light passing over observation glass. Replaces the old
 * memory-glitch squares. Drawn in the band's local (rotated) frame — the
 * caller tilts the container and clips it to the card. `t` runs 0 -> 1,
 * moving the glint corner to corner; outside that nothing is drawn.
 */
export const drawMirrorSweep = (g: Graphics, size: number, t: number) => {
	if (t <= 0 || t >= 1) return;
	// travel past both edges so the glint enters and leaves cleanly
	const x = (t * 2 - 1) * size * 0.9;
	// the band is tilted, so it must be tall enough to cover the diagonal
	const h = size * 2.2;
	const fade = Math.min(1, t / 0.12, (1 - t) / 0.12);
	const bands: [number, number][] = [
		[size * 0.55, 0.1], // wide soft halo
		[size * 0.26, 0.2], // body
		[size * 0.09, 0.55], // hot core
	];
	for (const [w, a] of bands) {
		g.rect(x - w / 2, -h / 2, w, h);
		g.fill({ color: 0xffffff, alpha: a * fade });
	}
	// thin secondary glint trailing the main band
	g.rect(x - size * 0.22, -h / 2, size * 0.03, h);
	g.fill({ color: WHITE_ROOM_PALETTE.bone, alpha: 0.4 * fade });
};
