...
<script lang="ts">
	...
	context.eventEmitter.subscribeOnMount({
		targetLockShow: async ({ cells, tone: incoming }) => {
			...
			// revolver cock / heavy metal click — not clinical anticipation bed
			context.eventEmitter.broadcast({ type: 'soundOnce', name: 'sfx_ui_click_heavy' });
			await lock.set(1, {
	...
	const drawMark = (g: import('pixi.js').Graphics, p: number) => {
		...
		// Wanted-poster iron corner ticks ONLY — no yellow/gold hairline on top.
		for (const sx of [-1, 1]) {
			for (const sy of [-1, 1]) {
				...
				g.lineTo(x, y);
				g.lineTo(x - sx * arm, y);
				g.stroke({ color: TOMBSTONE_FX.iron, width: 3.8, alpha: 0.85 + 0.15 * p });
				g.moveTo(x, y - sy * (arm - 4));
				g.lineTo(x, y);
				g.lineTo(x - sx * (arm - 4), y);
				g.stroke({ color: accent, width: 1.4, alpha: 0.55 * p });
			}
		}

		// Iron scope ring (dust/rust accent) — never neon brass.
		const radius = halfH * (1.15 - 0.5 * p);
		g.circle(0, 0, radius);
		g.stroke({ color: TOMBSTONE_FX.iron, width: 2.6, alpha: 0.55 * (1 - p * 0.25) });
		g.circle(0, 0, radius);
		g.stroke({ color: accent, width: 1.2, alpha: 0.4 * (1 - p * 0.2) });

		// Front-sight post while closing.
		if (!settled) {
			const postY = -halfH + p * CARD_H * 0.35;
			g.moveTo(-8, postY);
			g.lineTo(8, postY);
			g.stroke({ color: TOMBSTONE_FX.iron, width: 2.6, alpha: 0.65 });
			g.moveTo(0, postY - 10);
			g.lineTo(0, postY + 5);
			g.stroke({ color: accent, width: 1.6, alpha: 0.55 });
		}

		if (settled) {
			const breathe = 0.55 + 0.45 * Math.sin(time * 7);
			for (const sx of [-1, 1]) {
				g.moveTo(sx * (halfW - 5), -5);
				g.lineTo(sx * (halfW - 5), 5);
				g.stroke({ color: TOMBSTONE_FX.ironEdge, width: 2.2, alpha: 0.55 + 0.3 * breathe });
				g.moveTo(sx * (halfW - 14), 0);
				g.lineTo(sx * (halfW - 5), 0);
				g.stroke({ color: accent, width: 1.4, alpha: 0.4 + 0.25 * breathe });
			}
			// Powder wash only — NO yellow rounded outer border stroke.
			...
			g.stroke({ color: TOMBSTONE_FX.iron, width: 1.6, alpha: 0.35 + 0.2 * breathe });
		}
	};
</script>
