<script lang="ts">
	 ...
	 * Same particle loop as spine-scene/viewer.html (after the background
	 * slot, under posts and lamps).
	 */
	...
	const smoke: Smoke[] = [];
	const fire: Fire[] = [];
	const density = WESTERN_SCENE_FX.density;
	for (let i = 0; i < density; i += 1) {
		const p = {} as Smoke;
		resetSmoke(p, true);
		smoke.push(p);
	}
	const fireN = Math.max(8, Math.round(density * 0.45));
