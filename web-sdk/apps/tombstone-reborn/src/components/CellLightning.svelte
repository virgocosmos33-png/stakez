...
<!-- zIndex keeps the crackle ABOVE every stage sibling: overlays that mount
	mid-feature (StretchFx / StretchWays / WildReelSlide, the WinDim dark
	overlay...) are APPENDED to the stage and would otherwise cover it —
	pixi-svelte re-runs sortChildren on every attach, so an explicit zIndex on
	our stage-level node always wins over the default-0 siblings. The bolt is
	a transparent border trace, so sitting on top never hides a symbol. -->
{#if show}
	<Container zIndex={10}>
		<MainContainer>
			{@const now = time}
