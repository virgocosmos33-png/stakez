...
# OneDrive sometimes locks the target — write via temp then replace
_tmp = OUT.with_name(OUT.stem + ".tmp.png")
im2.save(_tmp, format="PNG")
try:
	_tmp.replace(OUT)
except OSError as e:
	_alt = OUT.with_name(OUT.stem + "_wr.png")
	im2.save(_alt, format="PNG")
	try:
		_tmp.unlink(missing_ok=True)
	except OSError:
		pass
	print(f"[warn] locked {OUT.name} ({e}); wrote {_alt.name}")
