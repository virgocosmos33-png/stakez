...
def replace_transition_rocks() -> None:
    """BLOCKED — Madam purple glass diamonds are banned on White Room.

    Use tools/make_transition_debris.py (pipeline scope=transition) instead.
    """
    raise SystemExit(
        "BLOCKED: replace_transition_rocks paints Madam purple diamonds. "
        "Run: python tools/make_transition_debris.py  (or regenerate_assets scope=transition)"
    )
...
if __name__ == "__main__":
    ...
    if "shards" in sys.argv:
        ...
        # Do NOT retheme transition/anticipation here — that path reintroduces
        # purple Madam diamonds. White Room uses make_transition_debris.py.
        print(
            "[retheme_mirror_debris] SKIP transition/anticipation "
            "(use make_transition_debris.py for White Room clinical debris)"
        )
        retheme_fs_intro()
        retheme_coin()
