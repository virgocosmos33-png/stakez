"""Build bonus side-character TAIL loop via seamless ping-pong (v9).
...
Source: last TAIL_S (1.0s) of lady_idle_bonus_v6.webm
  (Scenario asset_mmvxxR2qWvKd3CxSawDxw27q master).

Audio: best-effort ping-pong of the same 1.0s extract —
  forward audio + reverse audio with join-dup frame trimmed
  so duration matches (2N-1)/fps.

Ship: lady_idle_bonus_v9_loop.webm (cache-bust).

Run: python tools/_build_lady_idle_bonus_v9_loop.py
"""
...
QA = SRC_DIR / "qa_v9_loop"
FPS = 24
TAIL_S = 1.0
OUT_NAME = "lady_idle_bonus_v9_loop.webm"
...
def main() -> None:
    ...
    print(f"[bonus_v9_loop] src={src.name} dur={duration:.3f}s ss={start:.3f} tail={TAIL_S}", flush=True)

    work = Path(tempfile.mkdtemp(prefix="lady_bonus_v9_loop_"))
    raw_frames = work / "raw"
    pp_frames = work / "pp"
    ...
    try:
        ...
        print(f"[bonus_v9_loop] png start==end MSE={wrap_mse:.6f} (want ~0)", flush=True)
        print(f"[bonus_v9_loop] png join adjacent MSE={join_mse:.4f} (want >0)", flush=True)
        if wrap_mse > 0.5:
            raise RuntimeError(f"start/end frames do not match (MSE={wrap_mse})")

        dropped = scrub_png_dir(pp_frames)
        print(f"[bonus_v9_loop] island scrub dropped_px~{dropped}", flush=True)
        wrap_mse2 = frame_mse(first_png, last_png)
        if wrap_mse2 > 1.0:
            raise RuntimeError(f"scrub broke start/end match (MSE={wrap_mse2})")

        # Audio: same 1.0s window, then fwd + rev (join-dup frame trimmed).
        ...
        meta = probe(tmp_webm)
        print(f"[bonus_v9_loop] encoded {meta}", flush=True)
        ...
        if meta["frames"] < total:
            print(
                f"[bonus_v9_loop] WARN probe frames={meta['frames']} < {total}; "
                "relying on start==end MSE gate",
                flush=True,
            )

        first, last, webm_mse = extract_edge_frames(tmp_webm, QA, "bonus_v9_loop")
        print(f"[bonus_v9_loop] webm first==last MSE={webm_mse:.6f}", flush=True)
        if webm_mse > 5.0:
        ...
        for dest_dir in (VITE, STATIC):
            ...
            print(f"[bonus_v9_loop] installed {dest_dir / OUT_NAME}", flush=True)
        ...
        (SRC_DIR / "build_v9_loop_summary.json").write_text(
            json.dumps(summary, indent=2), encoding="utf-8"
        )
        note = SRC_DIR / "SOURCE_SCENARIO_V9_LOOP.txt"
        note.write_text(
            "BONUS LOOP v9: ping-pong of last 1.0s of lady_idle_bonus_v6\n"
            ...
            "  Audio: fwd + rev of same 1.0s extract (join-dup frame trimmed)\n"
            f"  Shipped: {OUT_NAME}\n"
            "  FE: SceneCharacter play full v6 once → SWAP to v9_loop loop=true\n",
            encoding="utf-8",
        )
