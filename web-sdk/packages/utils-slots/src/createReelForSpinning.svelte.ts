...
import { sineOut, backIn, backOut, linear } from 'svelte/easing';
...
export function createReelForSpinning<TRawSymbol extends object, TSymbolState extends string>(
	...
	const nudgeDown = async ({
		insertRawSymbol,
		durationMs = 260,
	}: {
		insertRawSymbol: TRawSymbol;
		durationMs?: number;
	}) => {
		// Prepend the incoming symbol, drop the bottom padding symbol, then shift
		// the whole reel up one cell and tween it back down: a single nudge step.
		const currentRawSymbols = targetSymbols.map((reelSymbol) => reelSymbol.rawSymbol);
		const newRawSymbols = [insertRawSymbol, ...currentRawSymbols.slice(0, -1)];
		const newSymbols = createReelSymbols(newRawSymbols);

		prevSymbols = [...newSymbols];
		targetSymbols = [...newSymbols];
		reelState.symbols = [...newSymbols];

		placeY(defaultY - reelOptions.symbolHeight);
		await reelY.set(defaultY, { duration: durationMs, easing: backOut });
	};
